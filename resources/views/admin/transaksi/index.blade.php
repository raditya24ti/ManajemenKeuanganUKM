...
@section('content')
    <div class="container py-5">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end mb-5 gap-3">
            ...
            <a href="{{ route('transaksi.create') }}" class="btn btn-primary px-4 py-2 shadow-blue fw-bold"
                style="border-radius: 12px; background: linear-gradient(45deg, #0d6efd, #00d4ff); border: none;">
            ...
            <div class="card-body p-4">
                <form action="{{ route('transaksi.index') }}" method="GET" class="row g-3 align-items-center">
                    <div class="col-md-3">
                        ...
                        <select name="jenis" class="form-select bg-dark text-white border-0 py-2 shadow-none"
                    ...
                    <div class="col-md-5">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="search"
                                class="form-control bg-dark text-white border-0 py-2 shadow-none"
                                style="border-radius: 0 10px 10px 0;" placeholder="Cari kategori atau keterangan..."
                    ...
                    <div class="col-md-4 d-flex align-items-end gap-2 pt-md-4">
                        ...
                        <a href="{{ route('transaksi.index') }}" class="btn btn-outline-secondary py-2 px-3"
                            style="border-radius: 10px;">
                        ...
                        @forelse($transaksi as $item)
                            <tr class="border-bottom border-secondary border-opacity-10">
                                <td class="ps-4 text-white-50">
                                    <div class="fw-bold text-white">{{ \Carbon\Carbon::parse($item->tanggal)->format('d') }}
                                    </div>
                                    <div class="small">{{ \Carbon\Carbon::parse($item->tanggal)->translatedFormat('M Y') }}
                                    </div>
                                </td>
                                <td class="text-center">
                                    @if ($item->jenis == 'masuk')
                                        <span class="badge rounded-pill"
                                            style="background: rgba(40, 167,  green 69, 0.1); color: #2ecc71; border: 1px solid rgba(46, 204, 113, 0.2);">
                                ...
                                <td>
                                    ...
                                    <div class="text-secondary x-small text-truncate" style="max-width: 150px;"
                                ...
                                <td class="text-center">
                                    @if ($item->bukti_pembayaran)
                                        <a href="{{ asset('storage/' . $item->bukti_pembayaran) }}" target="_blank"
                                            class="btn btn-sm btn-dark border-0 text-info" style="border-radius: 8px;">
                                ...
                                <td class="pe-4">
                                    <div class="d-flex justify-content-center gap-1">
                                        <a href="{{ route('transaksi.edit', $item->id) }}"
                                            class="btn btn-sm btn-icon-hover text-info">
                                        ...
                                        <form action="{{ route('transaksi.destroy', $item->id) }}" method="POST"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-icon-hover text-danger"
        ...
        <div class="mt-4 d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
            <div class="text-secondary small">
                Menampilkan <b>{{ $transaksi->firstItem() ?? 0 }}</b> - <b>{{ $transaksi->lastItem() ?? 0 }}</b> dari
                <b>{{ $transaksi->total() }}</b> transaksi
            </div>
