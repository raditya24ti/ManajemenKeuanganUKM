...
@section('content')
        ...
        <div class="row g-3">
                ...
                <div class="card bg-dark border-primary border-opacity-10 h-100 shadow-sm">
                    ...
                    <ul class="list-group list-group-flush bg-transparent">
                        @forelse($kategori as $k)
                            <li
                                class="list-group-item bg-transparent border-primary border-opacity-10 d-flex justify-content-between align-items-center text-white-50 py-3">
                                {{ $k->kategori }}
            ...
            <div class="col-md-7">
                <div class="card bg-dark border-primary border-opacity-10 h-100 shadow-sm overflow-hidden">
                    <div
                        class="card-header bg-transparent border-primary border-opacity-10 py-3 d-flex justify-content-between align-items-center">
                        <span class="text-white fw-bold"><i class="fas fa-history me-2 text-primary"></i>Aktivitas
                            Terbaru</span>
                        <a href="{{ route('transaksi.index') }}" class="btn btn-sm btn-outline-primary border-0">Lihat
                            Semua</a>
                    </div>
                                ...
                                @forelse($transaksiTerbaru as $t)
                                    <tr class="border-bottom border-primary border-opacity-5">
                                        ...
                                        <td class="text-center">
                                            <span
                                                class="badge rounded-pill bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 px-3">
                                                {{ ucfirst($t->jenis) }}
                                            </span>
                                        </td>
                                        <td class="text-end pe-3 fw-bold">Rp {{ number_format($t->jumlah, 0, ',', '.') }}
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-5 text-secondary small">Belum ada transaksi
                                            tercatat.</td>
                                    </tr>
                                @endforelse
                            </tbody>
    ...
    <style>
        ...
        /* Warna GitHub Dark sesuai template sebelumnya */
        .card {
